...
class Transform:
    ...
    def __init__(self, y_train):
        ...
        for y in y_train:
            ...
            transformed_y = offsets[int(round(y)) if y < 9 else 8] + K[int(round(y)) if y < 9 else 8] * (y - int(round(y)))
    ...
    def fit(self, y_test):
        if isinstance(y_test, Iterable):
            ...
            for y in y_test:
                ...
                transformed_y = self.offsets[int(round(y)) if y < 9 else 8] + self.K[int(round(y)) if y < 9 else 8] * (y -
                                                                                                            int(round(y)))
            ...
            if y_test is None:
                return None
            return self.offsets[int(round(y_test)) if y_test < 9 else 8] + self.K[int(round(y_test)) if y_test < 9 else 8] * (y_test -
                                                                                                            int(round(y_test)))
    ...
    def decode(self, Y):
        if isinstance(Y, Iterable):
            return np.array([self.__decode_value__(y) for y in Y])
